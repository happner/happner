...
// process.env.EXIT_ON_SIGINT (OR: it just stops components and disconnects network) <--- functionality pending
...
module.exports.start = function(config, callback) {
  ...
  // START_AS_ROOTED=1 node -e 'require("happner").start()'
  ...
  // START_AS_ROOTED=1 node -e 'require("happner").start(9999)'
  ...
  // START_AS_ROOTED=1 node -e 'require("happner").start("5.6.7.88:7654")'
...
Mesh.prototype.initialize = function(config, callback){
  ...
  // Async config step for (what should i do)ness <--- functionality pending
  ...
  Config.process(this, config, function(e, config) {
    ...
    if (process.env.START_AS_ROOTED) {
      ...
      $happner.nodes[config.name] = _this;
    }
    ...
    process.on('uncaughtException', function(err) {
      _this.log.fatal('uncaughtException, exiting! (or set NO_EXIT_ON_UNCAUGHT)', err);
    ...
    process.on('SIGINT', function() {
      console.log('');
      if (process.env.EXIT_ON_SIGINT) {
        _this.log.warn('SIGINT with exit');
        return stopmesh();
      }
