...
Mesh.prototype.getPackagedModules = function(config, callback){
  ...
  UTILITIES.findInModules('happner.js',
    function(e, matches){

      // Matches are returned sorted deepest to shallowest,
      // so that shallowest are processed last and therefore
      // overwrite into config.packaged[packageName] so
      // that the shallowest packages are used.

      // Pending, there is a version available, - use most recent??
      //
      ...
      matches.map(function(match){
        ...
        if (! happnerConfig.configs.autoload) {
          // No autoload. That's fine.
          return;
        }

        if (! Array.isArray(happnerConfig.configs.autoload)) {
          error = new Error('Expected Array in happner.js(.configs.autoload) in ' + happnerFile);
          return;
        }

        var seq = 0;

        happnerConfig.configs.autoload.forEach(function(elementConfig) {
          ...
          if (! elementConfig.component.config.module || elementConfig.component.config.moduleName) {
            elementConfig.component.config.module = elementConfig.module.name;
          }

          var parts = happnerFile.split(path.sep);
          
          elementConfig.depth = parts.length; // <----- not yet used
          elementConfig.packageName = elementConfig.component.name;

          //TODO - we want to be clever about finding the module path here...:

          if (!elementConfig.module.path)
            elementConfig.module.path = match.replace('/happner.js','');
          ...
          //
          //   I think it points to the wrong place.
          //   Also don't want to FORCE use of path
          //
          //   Put path only if you want path. And put it in module config as always.
          ...
          config.packaged[elementConfig.packageName] = elementConfig;
...
Mesh.prototype.loadPackagedModules = function(config, callback){
  ...
  for (var packageName in config.packaged){
                      //
                     // TODO: sometimes load order is important

    var loadConfig = config.packaged[packageName];

    if (loadConfig){

      // TODO: merge into possible existing config

      config.modules[loadConfig.module.name] = loadConfig.module.config;
      config.components[loadConfig.component.name] = loadConfig.component.config;
      // config.modules[loadConfig.module.name].packaged = packageName;
    }
  }
