...
Mesh.prototype.initialize = function(config, callback){
  ...
  Config.process(this, config, function(e, config) {
    ...
    process.on('SIGINT', function() {
      console.log();
      _this.log.$$DEBUG('SIGINT');
      if (process.env.NO_EXIT_ON_SIGINT) return;
...
Mesh.prototype.start = function(callback){
  if (!this.initialized) return _this.log.warn('missing initialize()');
...
Mesh.prototype.describe = function(cached){
  if (!this.initializing && !this.initialized) return _this.log.warn('beggaring description!');
