...
Mesh.prototype._createComponent = function(spec, callback) {
  ...
  var __addComponentDataStoreRoutes = function(meshConfig, componentConfig){
    ...
    if (componentConfig.datalayer && 
        meshConfig.datalayer && 
        meshConfig.datalayer.persist){//persist is set to true if datalayer.filename is set, or is also explicitly set

      for (var route in componentConfig.datalayer.routes){
        var componentRoute = '/_data/' + componentName + '/' + route.replace(/^\//,'');
        console.log('adding route:::', componentRoute);
        this._mesh.datalayer.server.services.data.addDataStoreFilter(componentRoute, componentConfig.datalayer.routes[route]);
      }
  ...
  //       var componentRoute = '/_data/' + componentName + '/' + route.replace(/^\//,'');
  //       console.log('adding route:::', componentRoute);
  //       this._mesh.datalayer.server.services.data.removeDataStoreFilter(componentRoute, componentConfig.datalayer.routes[route]);
  //     }
