...
function Mesh(config) {
  ...
  Object.defineProperty(this, 'api', {
    get: function() {
      ...
      try {
        _this.log.warn(' - at ' + _this.getCallerTo('mesh.js'));
      } catch (e) {}
      depWarned0 = true;
      return _this;
    }
      ...
      if (!home) {
        ...
        try {
          ...
          modulePath.replace(/\.js$/, '').split('.').map(function(part, ind){
            ...
            if (ind == 0){
              moduleBasePath = part;
              _this.log.$$DEBUG('requiring module ' + modulePath);
          ...
          try {
            _this.log.$$DEBUG('alt-requiring module happner-' + modulePath);
            moduleBase = require('happner-' + modulePath);
            moduleBasePath = 'happner-' + modulePath;
          } catch (f) {
            _this.log.$$DEBUG('alt-requiring happner-' + modulePath + ' failed', f);
            throw e
          }
        }
      }
