...
// process.env.SKIP_SUITES (don't perform suite loading from happner.js)
  ...
  configer.process(this, config, function(e, config) {
    ...
    async.series([
      ...
      function(callback) {
        ...
        _this.log.$$DEBUG('searched for suites');
        _this.loadPackagedModules(config, callback);
      },
      function(callback) {
        if (!process.env.SKIP_SUITES) {
          _this.log.$$DEBUG('initialized suites');
        }
      ...
      matches.map(function(match){
        ...
        if (happnerConfig.suite){//we have a bunch of packaged modules, nice to do a single npm install and get a selection of modules
          ...
          happnerConfig.suite.every(function(suiteItemConfig){
            ...
            if (!suiteItemConfig.name){
              callback(new Error('suite item name not specified for package: ' + match));
              return false;
            }
            
            suiteItemConfig.depth = match.split('/').length;
            suiteItemConfig.packageName = suiteItemConfig.name;

            if (!suiteItemConfig.path)
              suiteItemConfig.path = match.replace('/happner.js','') + '/' + suiteItemConfig.name;

            config.packaged[suiteItemConfig.packageName] = suiteItemConfig;
          ...
          happnerConfig.packageName = happnerConfig.name;
          config.packaged[happnerConfig.packageName] = happnerConfig;
