...
//lazy loaded dependancies
var DEP = {};

var MeshClient = require('./system/shared/mesh-client')
...
function Mesh(config) {
  ...
  this._mesh = {
    ...
    startupProgress:function(log, progress){
      if (this.startupProxyManager)
        this.startupProxyManager.progress(log, progress);
    }
...
Mesh.prototype.__initialize = function(config, callback){
  ...
  DEP.utilities = require('./system/utilities');
  ...
  _this._mesh.startupProgress("initializing data layer...", 10);
  ...
  _this._initializeDataLayer(config, function(e){
    ...
    _this._mesh.startupProgress("initialized data layer", 50);
    ...
    configer.process(_this, config, function(e, config) {
      ...
      _this._mesh.startupProgress("configured mesh", 55);
      ...
      if (!config.home) {
        config.home = config.home || _this._mesh.caller.file ? DEP.path.dirname(_this._mesh.caller.file) : null
      }
      ...
      DEP.repl = require('./system/repl');
      DEP.repl.create(_this);

      _this._mesh.startupProgress("created repl", 60);
      ...
      _this._mesh.startupProgress("attached system components", 65);

      DEP.async = require("async");
      ...
      DEP.async.series([
        ...
        function(callback) {
          ...
          _this._mesh.startupProgress("got packaged modules", 70);
        ...
        function(callback) {
          ...
          _this._mesh.startupProgress("got autoload modules", 75);
        ...
        function(callback) {
          ...
          _this._mesh.startupProgress("loaded component suites", 75);
        ...
        function(callback) {
          ...
          _this._mesh.startupProgress("registered initial schema", 80);
        ...
        function(callback) {
          ...
          _this._mesh.startupProgress("initialized packager", 80);

          var isServer = true;
          var describe = {};

          DEP.Internals = require('./system/shared/internals');

          DEP.Internals._initializeLocal(_this, describe, config, isServer, callback);
        },
        function(callback) {

          _this._mesh.startupProgress("initialized local", 80);
        ...
        function(callback) {
          ...
          _this._mesh.startupProgress("initialized elements", 90);
        ...
        function(callback) {
          ...
          _this._mesh.startupProgress("registered schema", 90);
        ...
        function(callback) {
          ...
          //   DEP.Internals._attachProxyPipeline(_this, _this.describe(), Happn, config, callback);
          // },
          // function(callback) {
...
Mesh.prototype.initialize = Promise.promisify(function(config, callback){
  ...
  if (config.startupProxy && config.startupProxy.enabled == true){
    ...
    var ProxyManager = require('./startup/proxy_manager');
    _this._mesh.startupProxyManager = new ProxyManager();

    if (!config.startupProxy.port)
      config.startupProxy.port = config.port;

    _this._mesh.startupProxyManager.start(config.startupProxy, function (e) {
  ...
  }else _this.__initialize(config, callback);
...
Mesh.prototype.start = Promise.promisify(function(callback){
  ...
  this._mesh.startupProgress("starting mesh", 90);
  ...
  this.__startComponents(function(error) {
    ...
    _this._mesh.startupProgress("mesh started", 100);

    if (_this._mesh.config.startupProxy && _this._mesh.config.startupProxy.enabled == true){

      // this._mesh.startupProxyManager
      _this._mesh.startupProxyManager.stop();

      DEP.async.retry({times:10, interval:1000}, _this._mesh.datalayer.listen, function(e) {
        if (e) return callback(e);
        callback(null, _this);
      });
...
Mesh.prototype.updateSearchPath = function(startAt) {
  ...
  var addPath = function(dir) {
    var add = DEP.path.normalize(dir + DEP.path.sep + 'node_modules');
...
Mesh.prototype._initializeDataLayer = function(config, callback){
  ...
  DEP.DataLayer = require('./system/datalayer')

  this._mesh.datalayer = DEP.DataLayer.create(this, config,
    function(err, client) {
...
Mesh.prototype._initializePackager = function(callback) {
  DEP.Packager = require('./system/packager');
  var packager = new DEP.Packager(this);
...
Mesh.prototype._destroyElement = Promise.promisify(function(componentName, callback) {
  ...
  DEP.async.series([
    ...
    function(done) {
      DEP.Internals._updateEndpoint(_this, endpointName, _this.exchange, _this.event, done);
    },
...
Mesh.prototype._createElement = Promise.promisify(function(spec, writeSchema, callback) {
  ...
  DEP.async.series([
    ...
    function(done) {
      DEP.Internals._updateExchangeAPILayer(_this, endpointName, _this.exchange, done);
    },
    function(done) {
      DEP.Internals._updateEventAPILayer(_this, endpointName, _this.event, done);
    },
    function(done) {
  ...
  try {
    ...
    if (!home) {
      ...
      try {
        ...
        var dirName = DEP.path.dirname(modulePath);
        var baseName = DEP.path.basename(modulePath);
        ...
        baseName.replace(/\.js$/, '').split('.').map(function(part, ind){
          ...
          if (ind == 0){
            ...
            if (dirName == '.' && modulePath[0] != '.') {
              ...
              moduleBasePath = part = dirName + DEP.path.sep + part;
            }
    ...
    home = home || DEP.path.dirname(require.resolve(moduleBasePath));
...
Mesh.prototype._happnizeModule = function(spec, callback){
  ...
  for(var fnName in module) {
    ...
    args = DEP.utilities.getFunctionParameters(originalFn);
...
Mesh.prototype._createComponent = function(spec, callback) {
  ...
  if (!DEP.ComponentInstance)
    DEP.ComponentInstance = require('./system/component-instance')

  var componentInstance = new DEP.ComponentInstance();
...
Mesh.prototype._initializeEndpoints = function(callback) {
  ...
  DEP.async.parallel(Object.keys(config.endpoints).map(function(endpointName) {
    ...
    return function(done) {
      ...
      DEP.Happn = require('happn');

      DEP.Happn.client.create(endpointConfig, function(error, client){
        ...
        var update = function(description) {
          ...
          DEP.Internals._updateEndpoint(_this, endpointName, exchangeAPI, eventAPI, function(e) {
            buzy = false;
...
Mesh.prototype.getPackagedModules = function(config, callback){
  ...
  DEP.utilities.findInModules('happner.js', module.paths,
    function(e, matches){
...
Mesh.prototype._loadComponentSuites = function(config, callback) {
  ...
  Promise.map(Object.keys(config.components), function(name) {
    ...
    if (component.$configure) {
      ...
      suiteName = DEP.utilities.getFunctionParameters(passthroughFunction)[0];
    ...
    var parts = modulePath.split(DEP.path.sep);
    var last = parts.pop();
    last = last.split('.').shift();
    parts.push(last);
    modulePath = parts.join(DEP.path.sep);
    ...
    happnerFile = DEP.path.dirname(resolved) + DEP.path.sep + 'happner.js';
