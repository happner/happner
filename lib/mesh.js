...
module.exports.PromiseIs = 'bluebird v' + require('../node_modules/bluebird/package.json').version;
...
function Mesh(config) {
  ...
  this._mesh = {
    ...
    initializing: false,  // true         - runlevel 1
    initialized: false,  // true          - runlevel 2
    starting: false,    // true           - runlevel 3
    started: false,    // true            - runlevel 4
  ...
  Object.defineProperty(this, 'runlevel', {
    get: function() {
      if (_this._mesh.started) return 4;
      if (_this._mesh.starting) return 3;
      if (_this._mesh.initialized) return 2;
      if (_this._mesh.initializing) return 1;
      return 0;
    },
    // set: function(n) {
    // 
...
Mesh.prototype.initialize = Promise.promisify(function(config, callback){
  ...
  configer.process(this, config, function(e, config) {
    ...
    if (!config.home) {
      ...
      _this.log.info('home ' + config.home);
    }

    // Need to add the caller's module path as first for module searches.
    _this.updateSearchPath(config.home);

    _this.log.info('happner v' + _this.version);
    _this.log.info('config v' + (config.version || '..'));
    _this.log.info('localnode \''+config.name+'\' at pid ' + process.pid);
      ...
      process.on('exit', function(code) {
        ...
        Object.keys(root.nodes).forEach(
          function(name) {
            ...
            root.nodes[name].log.info('exit ' + code);
          }
        )
      });
...
Mesh.prototype.start = Promise.promisify(function(callback){
  ...
  var waiting = setInterval(function() {
    Object.keys(_this._mesh.calls.starting).forEach(function(name) {
      _this.log.warn('awaiting startMethod \''+ name +'\'');
    });
  },10*1000);

  var impatient = setTimeout(function() {
    Object.keys(_this._mesh.calls.starting).forEach(function(name) {
      _this.log.fatal('startMethod \''+ name +'\' did not respond', new Error('timeout'));
    });
    _this.stop({
      kill: true,
      wait: 200
...
Mesh.prototype._initializeDataLayer = function(config, callback){
  ...
  this._mesh.datalayer = DataLayer.create(this, config,
    function(err, client) {
      ...
      // no good! need events on happn server / pubsub
      //
      // // log connections
...
Mesh.prototype._initializeModules = function(config, callback){
  ...
  // NB: If this functionality is move into another module the
  ...
  async.eachSeries(Object.keys(config.modules), function(moduleName, eachCallback) {
    ...
    try {
      ...
      if (ignore) {
        _this.log.info('missing or null module \''+ignore+'\'');
    ...
    var errorIfNull = function(module) {
      if (!module) {
        _this.log.warn('missing or null module \''+moduleName+'\'');
        return {};
      }
      return module;
    }
...
Mesh.prototype._instantiateComponents = function(config, callback){
  ...
  async.eachSeries(Object.keys(config.components), function(componentName, eachCallback) {
    ...
    componentConfig.meshPort = config.datalayer.port; // TODO: might not always be, 
                                                     //        get port and host from happn http server.address() 
    componentConfig.setOptions = config.datalayer.setOptions;

    _this.log.$$TRACE('created component \'' + componentName + '\'', componentConfig);
...
Mesh.prototype._registerSchema = function(config, callback){
  ...
  _this.log.$$TRACE('_registerSchema( description with name: ' + description.name, description);
        ...
        client.get('/mesh/schema/description', {}, function(error, response) {
          ...
          try {
            ...
            if (endpointName !== description.name) {
              _this.log.warn('endpoint \''+endpointName+'\' returned description for \''+description.name+'\'');
            }

            _this.log.$$TRACE('got description from endpoint \'' + endpointName + '\'', description);
...
Mesh.prototype._eachComponentDo = function(options, callback){
  ...
  this._eachComponent(options.flow, function(componentName, component, done){
    ...
    if (!config.schema || !config.schema.methods || !config.schema.methods[options.methodName]) {
      ...
      calls[call] = Date.now();
      _this.log.$$DEBUG('calling ' + options.methodCategory + ' \''+call+'\' as default async');
    ...
    if (methodConfig.type == "sync") {
      try{
        _this.log.$$DEBUG('calling ' + options.methodCategory + ' \''+call+'\' as configured sync');
    ...
    calls[call] = Date.now();
    _this.log.$$DEBUG('calling ' + options.methodCategory + ' \''+call+'\' as configured async');
