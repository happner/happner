   ...
   function(e, matches){
      ...
      matches.map(function(match){
        ...
        if (! happnerConfig.configs) {
          return callback(new Error('Missing happner.js(.configs) in ' + match));
        }

        if (! happnerConfig.configs.autoload) {
          return callback(new Error('Missing happner.js(.configs.autoload) in ' + match));
        }

        if (! Array.isArray(happnerConfig.configs.autoload)) {
          happnerConfig.configs.autoload = [happnerConfig.configs.autoload];
        }
