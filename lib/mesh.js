...
Mesh.prototype.initialize = function(config, callback){
  ...
  this.updateSearchPath(this.getCallerTo('mesh.js').file);
  ...
  configer.process(this, config, function(e, config) {
    ...
    _this.log.info('happner version ' + _this.version);
    _this.log.info('config version ' + config.version);
...
Mesh.prototype._initializeModules = function(config, callback){
  ...
  console.log('INNER', module.paths);
...
Mesh.prototype.attachSystemComponents = function(config){
  ...
  // config.modules.www = config.modules.www || {
  //   path:"system:www"
  // }

  config.components.www = config.components.www || {}
