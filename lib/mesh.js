...
//unable to inherit from EventEmitter in node v0.10

var __handlers = {};

var __emit = function(key, data){
  if (__handlers[key]) {
    __handlers[key].forEach(function(handler){
...
module.exports.on = function(key, handler){
  ...
  __handlers[key].push(handler);
  return key + "__" + (__handlers[key].length - 1).toString();
}

module.exports.off = function(id){
  try{
    var keys = id.split('__');
    var eventKey = keys[0];
    var eventHandlerIndex = parseInt(keys[1]);

    if (__handlers[eventKey] && __handlers[eventKey].length > eventHandlerIndex){
      __handlers[eventKey] = __handlers[eventKey].splice(eventHandlerIndex, 1);
      return true;
    }
    return false;

  }catch(e){
    throw new Error("error detaching from mesh event: ", e);
  }
...
Mesh.prototype.startupProgress = function(log, progress){
    ...
    configer.process(_this, config, function(e, config) {
      ...
      async.series([
        ...
        function(callback) {
          ...
          _this.startupProgress("loaded component suites", 75);
        ...
        function(callback) {
          ...
          _this.startupProgress("initialized packager", 80);
        ...
        function(callback) {
          ...
          _this.startupProgress("initialized local", 80);
        ...
        function(callback) {
          ...
          _this.startupProgress("registered schema", 90);
...
Mesh.prototype.start = Promise.promisify(function(callback){
  ...
  this.startupProgress("starting mesh", 90);
