...
Mesh.prototype._destroyElement = Promise.promisify(function(componentName, callback) {
  ...
  async.series([
    ...
    // - thinking about hotswap
    //      - calling _createElement(on existing name)
    //      - first does destroyElement()
    //      - then _createElement()
    function(done) {
      var interval;
    ...
    // Stop the component.
    function(done) {
      console.log('TODO, stop if started');
      done();
    },
    ...
    function(done) {
      _this._mesh.elements[componentName].component.instance._detatch(done);
      done();
    },
    ...
    function(done) {
      console.log('TODO, physically remove element');
      done();
    },
  ...
  async.series([
    ...
    function(done) {
      ...
      _this._eachComponentDo({
        ...
        list: [spec.component.name], // only start this component
        log: 'started'
      }, done);
...
Mesh.prototype._eachComponent = function(list, flow, operator, callback){
  var _this = this;
  async[flow](
    Object.keys(this._mesh.elements)

    .filter(function(componentName) {
      return list.indexOf(componentName) != -1;
    })
...
Mesh.prototype._eachComponentDo = function(options, callback){
  ...
  if (!options.list) options.list = Object.keys(this._mesh.elements);
  ...
  this._eachComponent(options.list, options.flow, function(componentName, component, done){
