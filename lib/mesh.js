...
Mesh.prototype.initialize = Promise.promisify(function(config, callback){
  ...
  configer.process(this, config, function(e, config) {
    ...
    async.series([
      ...
      function(callback) {
        _this.loadPackagedModules(config, callback);
      },
      function(callback) {
      ...
      function(callback) {
        _this.log.$$DEBUG('initialized data layer');
                                    ...
                                    // At this time clients and other nodes can already be
                                   // attaching here, and getting a Bad/Old Descirptoin becuause
                                  // none has yet been set by this newly started node.
                                 //
                                // ie. This mesh is already listening but not ready...
                               //
                              // So this sets an initialDescription that says so, clients
                             // and other nodes subscribe, notice the initializing=true flag, 
                            // and await the full description that should shortly follow.
                           // 
                          // Currently this waiting behaviour is not configurable.
                         // A mesh node will wait forever for it's endpoints to be 
                        // fully described before continuing with the start methods.
                       // 
                      // Similarly, the browser client, once connected, will also wait 
                     // forever for a full description.
                    //
                   // TODO: The browser client can begin accessing services before
                  //        the startMethods are completed.
                 //      
      ...
      function(callback) {
        ...
        var isServer = true;
        var describe = {}; 
                           // Create event and exchange infrastructure
                          // ahead of element initialization to allow
                         // each new component insertion to assemble 
                        // it's partial onto the api's, this then
                       // allows for insertion of additional components
                      // at runtime that can walk the same code path
                     // all the way to assembling into the event and
                    // exchange api's
...
Mesh.prototype._createModule = function(spec, callback) {
  ...
  if (spec.module.config.packageId && this._mesh.config.packaged[spec.module.config.packageId])
    spec.module.packaged = this._mesh.config.packaged[spec.module.config.packageId];
    ...
    if (!home) {
      ...
      try {
        ...
        modulePath.replace(/\.js$/, '').split('.').map(function(part, ind){
                                          //
                                         // BUG: there may be dots in the path
                                        //       that are other than nested
                                       //        in the basename as property
                                      //         of module.
...
Mesh.prototype._createComponent = function(spec, callback) {
  ...
  var componentInstance = new ComponentInstance({name: componentName, mesh: this});
...
Mesh.prototype._registerInitialSchema = function(config, callback) {
  ...
  this.data.set('/mesh/schema/description', description, function(e, response){
    if (e) return callback(e);
    _this.data.set('/mesh/schema/config', _this._filterConfig(config), function(e, response){
      callback(e);
    });
  });
}
  ...
  UTILITIES.findFiles(
   ...
   function(e, matches){
      ...
      matches.map(function(match){
        ...
        var happnerConfig = require(match);

        if (happnerConfig.suite){//we have a bunch of packaged modules, nice to do a single npm install and get a selection of modules

          happnerConfig.suite.every(function(suiteItemConfig){

            if (!suiteItemConfig.id){
              callback(new Error('suite item id not specified for package: ' + match));
              return false;
            }
            
            suiteItemConfig.depth = match.split('/').length;
            suiteItemConfig.packageId = suiteItemConfig.id;

            if (!suiteItemConfig.path)
              suiteItemConfig.path = match.replace('/happner.js','') + '/' + suiteItemConfig.id;

            config.packaged[suiteItemConfig.packageId] = suiteItemConfig;
          ...
          if (!happnerConfig.id)
            return callback(new Error('id not specified for package: ' + match));

          happnerConfig.depth = match.split('/').length;

          if (!happnerConfig.path)
            happnerConfig.path = match.replace('/happner.js','');

          happnerConfig.packageId = happnerConfig.id;
          config.packaged[happnerConfig.packageId] = happnerConfig;
...
Mesh.prototype.loadPackagedModules = function(config, callback){
  
  for (var packagedConfigId in config.packaged){
    var packagedConfig = config.packaged[packagedConfigId];

    if (packagedConfig.autoload){

      if (!packagedConfig.autoload.module)
        packagedConfig.autoload.module = {};

      if (!packagedConfig.autoload.module.path)
        packagedConfig.autoload.module.path = packagedConfig.path;

      config.modules[packagedConfigId] = packagedConfig.autoload.module;
      config.modules[packagedConfigId].packaged = packagedConfigId;

      config.components[packagedConfigId] = packagedConfig.autoload.component;
