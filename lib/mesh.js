...
Mesh.prototype.initialize = Promise.promisify(function(config, callback){
  ...
  this.updateSearchPath(this._mesh.caller.file);
  ...
  configer.process(this, config, function(e, config) {
    ...
    _this.log.$$DEBUG('caller ' + _this._mesh.caller.file);
    ...
    if (Object.keys(root.nodes).length == 1) { // i believe that with node it is impossible
      ...
      // only one listener for eac below 
...
Mesh.prototype.getCallerTo = function(skip) {
  ...
  try {
    ...
    for (var i = 0; i < stack.length; i++) {
      ...
      if (parts.indexOf('bluebird')) continue;
