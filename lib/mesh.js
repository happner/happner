...
Mesh.prototype.__initialize = function (config, callback) {
  ...
  _this.startupProgress = function (log, progress) {
    if (_this.log && _this.log.info) _this.log.info('startup progress: ' + log, progress);
  ...
  _this.startupProgress("initializing data layer...", 56);
  ...
  _this._initializeDataLayer(config, function (e) {
    ...
    _this.startupProgress("initialized data layer", 63);
    ...
    configer.process(_this, config, function (e, config) {
      ...
      _this.startupProgress("configured mesh", 64);
      ...
      _this.startupProgress("created repl", 65);
      ...
      _this.startupProgress("attached system components", 66);
      ...
      async.series([
        ...
        function (callback) {
          ...
          _this.startupProgress("got packaged modules", 67);
        ...
        function (callback) {
          ...
          _this.startupProgress("got autoload modules", 68);
        ...
        function (callback) {
          ...
          _this.startupProgress("loaded component suites", 69);
        ...
        function (callback) {
          ...
          _this.startupProgress("registered initial schema", 70);
        ...
        function (callback) {
          ...
          _this.startupProgress("initialized packager", 78);
        ...
        function (callback) {
          ...
          _this.startupProgress("initialized local", 79);
        ...
        function (callback) {
          ...
          _this.startupProgress("registered schema", 91);
...
Mesh.prototype.start = Promise.promisify(function (callback) {
  ...
  _this.startupProgress("starting mesh", 92);
  ...
  _this.__startComponents(function (error) {
    ...
    // if we need to listen with deferListen and no loader, now is the time
    if (_this._mesh.config.deferListen && !_this._mesh.config["happner-loader"])
    {
      _this._mesh.datalayer.listen(function(err){
        callback(err, _this);
      });
      return;
    }
...
Mesh.prototype._destroyElement = Promise.promisify(function (componentName, callback) {
  ...
  async.series([
    ...
    // Unclear... Question:
    //
    // - Should the component stopMethod be called before or after detaching it from the mesh.
    // - Similar to startMethod, i think the case can be made for both. Suggesting 2 kinds of
    //   stopMethods:
    //
    //         beforeDetach: can still tell clients
    //         afterDetach: final cleanup,
    //                      no longer clients present to affect state/data during cleanup
    //
    // Decided to put it after detach for now. It can still 'talk' to other mesh components
    ...
    return function (done) {
      ...
      Happn.client.create(endpointConfig, function (error, client) {
        ...
        client.onEvent('reconnect-successful', function (eventData) {
          ...
          _this._mesh.endpoints[client.__endpointName].messenger.updateSession(client.session);
