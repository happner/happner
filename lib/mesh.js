  ...
  UTILITIES.findInModules('happner.js', module.paths,
    function(e, matches){
      ...
      var promise = Promise.map(matches, function(match) {
        ...
        if (! happnerConfig.configs[config.autoload]) {
        ...
        if (typeof happnerConfig.configs[config.autoload] == 'function') {
          return happnerConfig.configs[config.autoload](config).then(function(suite) {
            return ammendSuites(suite);
          });
        }

        return ammendSuites(happnerConfig.configs[config.autoload]);
