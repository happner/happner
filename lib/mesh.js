...
Mesh.prototype.getPackagedModules = function(config, callback){
  ...
  UTILITIES.findFiles(
   ...
   //then go up 2 directories and look at sibling directories
   ...
   function(e, matches){
      ...
      matches.map(function(match){
        ...
        if (! happnerConfig.configs) {
          error = new Error('Missing happner.js(.configs) in ' + match);
          return;
        }
        ...
        if (! Array.isArray(happnerConfig.configs.autoload)) {
          error = new Error('Expected Array in happner.js(.configs.autoload) in ' + match);
          return;
        }
        ...
        happnerConfig.configs.autoload.forEach(function(elementConfig) {
          ...
          var parts = match.split(path.sep);
          ...
          elementConfig.depth = parts.length;
          ...
          //       elementConfig.module.config.path = match.replace('/happner.js','') + '/' + elementConfig.module.name;
          //      
