...
Mesh.prototype.initialize = Promise.promisify(function(config, callback){
  ...
  configer.process(this, config, function(e, config) {
    ...
    async.series([
      function(callback) {
        _this.attachPackagedComponents(config, callback);
      },
      function(callback) {
...
Mesh.prototype.attachPackagedComponents = function(config, callback){
  if (!config.modules) config.modules = {};
  if (!config.components) config.components = {};

  console.log('attachPackagedComponents');
  ...
  UTILITIES.findFiles([path.resolve(__dirname, '../node_modules') + '/**/package.json',
                       path.resolve(__dirname, '../../node_modules') + '/**/package.json'], 
                       {}, 
                       function(e, matches){
                          if (e) return callback(e);
                          console.log(matches);
