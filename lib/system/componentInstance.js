...
function ComponentInstance(opts) {
  ...
  Object.defineProperty(this, 'mesh', {
    get: function() {
      ...
      try {
        _this.log.warn(' - at ' + opts.mesh.getCallerTo('componentInstance.js'));
      } catch (e) {}
      depWarned0 = true;
      return _this;
    }
...
ComponentInstance.prototype.initialize = function(root, mesh, module, config, callback){
  ...
  else if (config.accessLevel == 'mesh') {
    this._mesh = mesh;
  }
...
ComponentInstance.prototype.attachRouteTarget = function(meshRoutePath, componentRoutePath, targetMethod, route){
  ...
  if (targetMethod == 'static'){
    ...
    if (this.name == 'www' && route == 'static') {
      componentRoutePath = '/';
      this.log.$$TRACE('serving www ' + directory);
      this.log.$$TRACE(' - at ' + componentRoutePath);
      serve = serveStatic(directory);
  ...
  if (typeof methodDefn.$happnSeq !== 'undefined') {
    ...
    if (this.name == 'www') {
      ...
      if (route == 'global') {
        this.log.$$TRACE('global $happn method ' + targetMethod + '()');
  ...
  if (this.name == 'www') {
    if (route == 'global') {
      this.log.$$TRACE('global _ method ' + targetMethod + '()');
      serve = methodDefn.bind(context);
      this.data.context.connect.use(serve);
      return;
    }
    componentRoutePath = '/' + route;
    this.log.$$TRACE('service _ method ' + targetMethod + '()');
    this.log.$$TRACE('- at ' + componentRoutePath);
  }


  this.log.$$TRACE('serving _ method ' + targetMethod + '()');
  this.log.$$TRACE(' - at ' + componentRoutePath);
  this.log.$$TRACE(' - at ' + meshRoutePath);
...
ComponentInstance.prototype._attach = function(config, callback){
  ...
  this.data.on(subscribeMask, {event_type:'set'}, function(publication){
      ...
      _this.log.$$TRACE('received request at ' + subscribeMask);
      ...
      _this.operate(method, args, function(e, responseArguments){
        ...
        if (e) {
          ...
          _this.log.$$TRACE('operate( data.set( error ' + message.callbackAddress);
        ...
        _this.log.$$TRACE('operate( data.set( result ' + message.callbackAddress);
...
ComponentInstance.prototype.README = function() {/*
  ...
  injectied into functions as **$happn**.

  **$local** and **$happn** are the same thing.
  ...
  **The name of the mesh node this component is running in**

      __node> $local.info.mesh.name
      'home'

  **The internet address of this mesh node**

      __node> $local.info.datalayer.address
      { address: '127.0.0.1', family: 'IPv4', port: 55000 }
  
  **This component's config**
      ...
      { moduleName: 'terminal',
        ...
        schema: 
         { exclusive: true,
           methods: { register: [Object], start: [Object] } } }

  **Access to the mesh node this component is running in**

  Only available in ComponentInstance if accessLevel is 'mesh' or 'root'
  ...
  **Access to root**
  ...
  Only available in ComponentInstance if accessLevel is 'root'. This enables 
  access to all mesh nodes in the current process.

      // start 3 mesh mesh nodes in this process

      __node> Happner.start(port = 10000)
      __node> Happner.start(10001)
      __node> Happner.start(10002)

      // here they are

      __node> Object.keys($local._root.nodes)
      [ 'home', 'smallreaper', 'rubyroar', 'dunepython' ]
