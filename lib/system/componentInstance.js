...
ComponentInstance.prototype.getMethod = function(methodName, nocache){

  if (!nocache && this._methodCache[methodName]){
  ...
  if (methodName.indexOf('.') > -1){
    ...
    method = this.module.instance[methodParts[0]];

    methodParts.map(function(methodPart, methodPartIndex){
      if (methodPartIndex > 0)
        method = method[methodPart];
    });

  }else{
    method = this.module.instance[methodName];
  }

  if (method){
    this._methodCache[methodName] = method;
    ...
    value: function(methodName, parameters, callback){
      try{
        ...
        if (methodName.indexOf('.') > 0){
          var methodTree = methodName
        }

        var methodDefn = _this.getMethod(methodName);
        var error;
