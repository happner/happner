...
function ComponentInstance() {
  ...
  _this._loadModule = function(module){
    ...
    _this.operate = function(method, parameters, callback){
      try{
        ...
        console.log(_this.stats.component[_this.name]);
  ...
  _this.attachRouteTarget = function(meshRoutePath, componentRoutePath, targetMethod, route){
    ...
    console.log('methodDefn', methodDefn, targetMethod);

    if (targetMethod == 'static'){
      console.log('so it is static');
  ...
  _this._attach = function(config, callback){
    ...
    if (config.web){
      try{
        ...
        UTILITIES.log("Failure to attach modules web methods to component: " + _this.name, "error", e);
        return callback(e);
      }
    }
