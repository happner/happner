  ...
  if (!cached || !this.description) {
    ...
    for (var methodName in this.module.instance){
      ...
      if (methodName.indexOf('_') != 0 && typeof method == 'function'){
        if (method.$happngin && method.$happngin.ignore) {
          if (this.config.schema && _this.config.schema.methods) {
            if (!this.config.schema.methods[methodName]) {
              continue;
            }
          } else continue;
        }
        ...
        if (this.config.schema) {
          if (this.config.schema.methods) {
            var methodSchema = this.config.schema.methods[methodName];

            if (this.config.schema.exclusive && !methodSchema)
              continue;

            if (methodSchema){
              this.description.methods[methodName] = methodSchema;

              if (!methodSchema.parameters) {
                this._defaultParameters(method, methodSchema);
              }
              continue;
            }
          } else {
            if (this.config.schema.exclusive) {
              continue; //exclustive and no methodSchema
            }
          }
        }

        this.description.methods[methodName] = {"parameters":[]};
        this._defaultParameters(method, this.description.methods[methodName]);
      }
    }
  }
...
ComponentInstance.prototype._loadModule = function(module){
  ...
  var _this = this;
  this.operate = function(method, parameters, callback){
    try{

      var methodSchema = _this.description.methods[method];
      ...
      _this.log.error('Call to method ' + method + ' failed', error);
...
ComponentInstance.prototype.attachRouteTarget = function(meshRoutePath, componentRoutePath, targetMethod, route){
  ...
  if (targetMethod == 'static'){
    if (this.name == 'resources' && route == 'resources') {
      ...
      this.mesh.data.context.connect.use(meshRoutePath.replace(/\/resources$/,''), serve);
      this.mesh.data.context.connect.use(componentRoutePath.replace(/\/resources$/,''), serve);
      return;
    };
    directory = this.module.directory + '/' + route;
    serve = serveStatic(directory);
    this.mesh.data.context.connect.use(meshRoutePath, serve);
    this.mesh.data.context.connect.use(componentRoutePath, serve);
    return;
  }
...
ComponentInstance.prototype._attach = function(config, callback){
  ...
  this.mesh.data.on(listenAddress + '*', 
    {event_type:'set'}, 
