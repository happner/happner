  ...
  if (!cached || !this.description) {
    ...
    for (var methodName in this.module.instance){
      ...
      if (methodName.indexOf('_') != 0 && typeof method == 'function'){
        if (method.$happner && method.$happner.ignore) {
          if (this.config.schema && this.config.schema.methods) {
...
ComponentInstance.prototype.attachRouteTarget = function(meshRoutePath, componentRoutePath, targetMethod, route){
  ...
  if (targetMethod == 'static'){
    if (this.name == 'resources' && route == 'resources') {
      ...
      this.log.$$DEBUG('serving static ' + directory);
      this.log.$$DEBUG(' - at ' + componentRoutePath);
      this.log.$$DEBUG(' - at ' + meshRoutePath);
  ...
  if (typeof methodDefn.$happnSeq !== 'undefined') {
    ...
    this.log.$$DEBUG('serving $happn method' + targetMethod + '()');
    this.log.$$DEBUG(' - at ' + componentRoutePath);
    this.log.$$DEBUG(' - at ' + meshRoutePath);
  ...
  this.log.$$DEBUG('serving . method ' + targetMethod + '()');
  this.log.$$DEBUG(' - at ' + componentRoutePath);
  this.log.$$DEBUG(' - at ' + meshRoutePath);
