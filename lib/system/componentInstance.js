...
ComponentInstance.prototype.describe = function(cached){
  ...
  if (!cached || !this.description) {
    ...
    var routes, defn;
                      // TODO: include www routes in it's description
                     //
    if (this.name != 'www' && this.config.web && (routes = this.config.web.routes)) {
      for (var routePath in this.config.web.routes) {
        //if (routePath == 'global') continue;

        var route = routes[routePath];            // use last in route array
        if (route instanceof Array) route = route.slice(-1).pop();

        // if (this.name == 'www' ) {routePath = '/'}

        if (routePath == 'resources' && this.name == 'resources') {
          routePath = '/' + routePath;
        } 
        else {
