...
function ComponentInstance() {
        ...
        if (methodName.indexOf('_') != 0 && typeof method == 'function'){
          ...
          if (method.$happngin && method.$happngin.ignore) {
            if (_this.config.schema && _this.config.schema.methods) {
              if (!_this.config.schema.methods[methodName]) {
                continue;
              }
            } else continue;
          }
  ...
  _this.log = UTILITIES.createLogger(_this.name);

  _this.attachRouteTarget = function(meshRoutePath, componentRoutePath, targetMethod, route){
    var directory, serve, methodDefn = _this.module.instance[targetMethod];

    if (targetMethod == 'static'){
      if (_this.name == 'resources' && route == 'resources') {
      ...
      directory = _this.module.directory + '/' + route;
      serve = serveStatic(directory);
