...
ComponentInstance.prototype.secureData = function(meshData, meshName, componentName){
  
  var securedMeshData = {};
  securedMeshData.basePath = '/mesh/system/data/' + meshName + '/' + componentName;
  ...
  securedMeshData.off = function(listenerRef, done){
    ...
    return meshData.off(this.getPath(path), done);
  }
...
ComponentInstance.prototype.initialize = function(name, root, mesh, module, config, callback){
  ...
  try{
    ...
    this.data = this.secureData(mesh._mesh.data, mesh._mesh.config.name, this.name);
...
ComponentInstance.prototype._attach = function(config, mesh, callback){
  ...
  _this.emit = function(key, data, callback){
    ...
    var eventKey = '/events/' + _this.info.mesh.name + '/' + _this.name + '/';
