...
function System() {
	...
	_this.initialize = function($happn, callback){
		$happn.log.info("http://localhost:"+$happn.config.meshPort+"/system/dashboard");
		callback(null);
	}
	...
	_this.getDBFileInfo = function($happn, callback){
		...
		$happn.log.info("getting file stats: ");
		$happn.log.info($happn.data.context.services.data.config.dbfile);
	...
	_this.getStats = function($happn, measureInterval, callback){
		...
		try{
			...
			$happn.log.info("getting file getDBFileInfo: ");
