...
function Packager(mesh) {
  ...
  happnProtocol = require(require.resolve('happn').replace('/lib/index.js', '') + '/package.json').protocol;

  var script4 = normalize(__dirname + '/shared/logger.js');
  var script5 = normalize(__dirname + '/shared/promisify.js');
  var script6 = normalize(__dirname + '/shared/mesh-error.js');
  var script7 = normalize(__dirname + '/shared/messenger.js');
  var script8 = normalize(__dirname + '/shared/internals.js');
  var script9 = normalize(__dirname + '/shared/mesh-client.js');
...
Packager.prototype.assemble = function () {
  ...
  return new Promise(function (resolve, reject) {
    ...
    _this.scripts.forEach(function (script) {
      ...
      if (script.file.indexOf('client/base.js') > -1)
        scriptStr = scriptStr.replace('{{protocol}}', happnProtocol);
