...
Mesh.prototype.initialize = function(config, callback){
  ...
  async.series([
    ...
    function(callback) { _this.log.$$DEBUG('initialized data layer'    ); _this._initializeModules(config, callback) },
    function(callback) { _this.log.$$DEBUG('initialized modules'       ); _this._happnizeModules(callback) },
    function(callback) { _this.log.$$DEBUG('happnized modules'         ); _this._instantiateComponents(config, callback) },
    function(callback) { _this.log.$$DEBUG('initialized components'    ); _this._registerSchema(config, callback) },
    function(callback) { _this.log.$$DEBUG('registered schema'         ); API._initializeEndpoints(_this, _this.describe(), happn, config, callback) },
    function(callback) { _this.log.$$DEBUG('initialized endpoints'     ); API._createExchangeAPILayer(_this, callback) },
    function(callback) { _this.log.$$DEBUG('initialized exchange api'  ); API._createEventAPILayer(_this, callback) },
    function(callback) { _this.log.$$DEBUG('initialized events api'    ); API._attachProxyPipeline(_this, _this.describe(), happn, config, callback) },
    function(callback) { _this.log.$$DEBUG('attached to proxy pipeline'); callback() }
  ], function(e){
...
Mesh.prototype.describe = function(cached){
  if (this.description && cached == true) return this.description;

  var description = {"name":this.config.name, "components":{}, "setOptions":this.config.dataLayer.setOptions};

  for (var componentName in this.components){
    description.components[componentName] = this.components[componentName].instance.describe();
  }
...
Mesh.prototype._happnizeModules = function(callback){
  var _this = this;
  async.eachSeries(Object.keys(this.modules), function(moduleName, eachCallback) {
...
Mesh.prototype._registerSchema = function(config, callback){
  ...
  var _this = this;
  this.api.data.set('/mesh/schema/description', this.describe(false), null, function(e, response){
    if (e) return callback(e);
...
Mesh.prototype._eachComponent = function(operator, callback){
  var _this = this;
   async.eachSeries(Object.keys(this.components), function(componentName, eachCallback){
