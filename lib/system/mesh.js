...
function Mesh() {
  ...
  //     if (options.methodName && config.schema.methods[options.methodName]){
  ...
  //         });
  //       }
  //     }else if (options.failMissingMethod){
  //        eachCallback(new MeshError("on method " + options.methodName + " not found for component " + componentName));
  //     }else eachCallback();
  //   });
  // }
  ...
  _this.initialize = function(config, callback){
    ...
    async.series([
      ...
      function(callback) { UTILITIES.log('initialized modules'     ); _this._instantiateComponents(config, callback) },
