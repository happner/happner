...
$debug('required lib/system/mesh.js');
...
function Mesh() {
    ...
    async.eachSeries(Object.keys(config.modules), function(moduleName, eachCallback) {
      ...
      if (moduleConfig.construct) {
        ...
        console.log('constructed');
         console.log(_this.modules[moduleName].instance);
  ...
  _this._instantiateComponents = function(config, callback){
    ...
    async.eachSeries(Object.keys(config.components), function(componentName, eachCallback) {
      ...
      _this._stats.component[componentName] = {errors:0, calls:0};
  ...
  _this.initialize = function(config, callback){
    ...
    $debug('create utilities');

    global.UTILITIES = new Utilities(config.util);

    _this.log = UTILITIES.createLogger('mesh');

    _this.log.info('! start');

    _this.config = config;

    $debug('attach system components');
    ...
    async.series([
      ...
      function(callback) { _this.log.info('initialized data layer'    ); _this._initializeModules(config, callback) },
      function(callback) { _this.log.info('initialized modules'       ); _this._happnizeModules(callback) },
      function(callback) { _this.log.info('happnized modules'         ); _this._instantiateComponents(config, callback) },
      function(callback) { _this.log.info('initialized components'    ); _this._registerSchema(config, callback) },
      function(callback) { _this.log.info('registered schema'         ); API._initializeEndpoints(_this, _this.describe(), happn, config, callback) },
      function(callback) { _this.log.info('initialized endpoints'     ); API._createExchangeAPILayer(_this, callback) },
      function(callback) { _this.log.info('initialized exchange api'  ); API._createEventAPILayer(_this, callback) },
      function(callback) { _this.log.info('initialized events api'    ); API._attachProxyPipeline(_this, _this.describe(), happn, config, callback) },
      function(callback) { _this.log.info('attached to proxy pipeline'); callback() }
