...
module.exports.create = function (mesh, config, callback) {
  ...
  if (config.activateSessionManagement) config.datalayer.activateSessionManagement = config.activateSessionManagement;
  if (config.logSessionActivity) config.datalayer.logSessionActivity = config.logSessionActivity;
  if (config.sessionActivityTTL) config.datalayer.sessionActivityTTL = config.sessionActivityTTL;
  if (config.transformMiddleware) config.datalayer.transformMiddleware = config.transformMiddleware;
  ...
  if (typeof config.port !== 'undefined') {
    ...
    if (typeof config.datalayer.port == 'undefined') config.datalayer.port = config.port;

    delete config.port;
  }

  if (typeof config.datalayer.port == 'undefined') config.datalayer.port = 55000;
  ...
  if (config.datalayer.dbfile && !config.datalayer.filename) config.datalayer.filename = config.datalayer.dbfile;
  ...
  if (!config.datalayer.transport) config.datalayer.transport = {mode: 'http'};

  if (config.datalayer.secure) {

    if (!config.datalayer.middleware) config.datalayer.middleware = {};

    if (!config.datalayer.middleware.security) config.datalayer.middleware.security = {};
    ...
    services: {
      security: {
        ...
        config: {
          ...
          activateSessionManagement:config.datalayer.activateSessionManagement,
          logSessionActivity:config.datalayer.logSessionActivity,
          sessionActivityTTL:config.datalayer.sessionActivityTTL,
          profiles:config.datalayer.profiles,
          adminUser: adminUser,
      ...
      pubsub: {
        ...
        config: config.datalayer.pubsub || {}
      }
    },
  };

  if (config.datalayer.transformMiddleware) datalayerConfig.services.pubsub.config.transformMiddleware = config.datalayer.transformMiddleware;

  if (config.datalayer.deferListen) datalayerConfig.deferListen = config.datalayer.deferListen;

  if (config.datalayer.plugin) datalayerConfig.services.data.path = config.datalayer.plugin;

  if (config.datalayer.config) datalayerConfig.services.data.config = config.datalayer.config;
  ...
  if (config.name) datalayerConfig.name = config.name;

  if (config.datalayer.middleware) datalayerConfig.middleware = config.datalayer.middleware;

  if (config.datalayer.encryptPayloads) datalayerConfig.encryptPayloads = config.datalayer.encryptPayloads;

  if (config.datalayer.keyPair) datalayerConfig.keyPair = config.datalayer.keyPair;

  if (config.datalayer.accountLockout) {

    if (!datalayerConfig.services.security) datalayerConfig.services.security = {};

    if (!datalayerConfig.services.security.config) datalayerConfig.services.security.config = {};

    datalayerConfig.services.security.config.accountLockout = config.datalayer.accountLockout;
  }

  var onMiddleware = {

    incoming:function(packet, next){
      //check for listeners on _responses
      try{
        if (packet.data.action === 'on'){

          var path = utils.splitAndCut(packet.data.path, '@', 1);

          if (path.match(/^\/_exchange\/responses\/[a-zA-Z0-9-_]*\/[a-zA-Z0-9-_]*\/[a-zA-Z0-9-_]*\/[a-zA-Z0-9-_]{0,30}\*/))
            return next(new Error('wildcard listens on _exchange responses are forbidden, bad path: ' + path));
        }
      }catch(e){
        log.warn('error processing incoming packet: ' + e);
      }
  ...
  if (!datalayerConfig.services.pubsub.config.transformMiddleware) datalayerConfig.services.pubsub.config.transformMiddleware = [];

  datalayerConfig.services.pubsub.config.transformMiddleware.push({instance:onMiddleware});
      ...
      store.server.services.pubsub.on('disconnect', function (ev) {
        ...
        try {
          if (!ev.info._browser) {
            ...
            if (ev.info.mesh) {
              // another mesh logging in with endpoint
              name = ev.info.mesh.name;
            } else if (ev.info.happn) {
              // info.happn.name is login target, not source, ie. me (used on logging login to self)
              name = ev.info.happn.name
            } else {
              name = 'anonymous';
            }
        ...
        //log.$$TRACE('detach', ev.info);
        store.events.emit('detach', ev);  // correct spelling
        store.events.emit('detatch', ev); // keep for backward compatibility
      });
