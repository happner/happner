;(function (isBrowser) {
  ...
  Messenger.prototype.updateSession = function(session){
    ...
    _this.session = {"id": session.id};
    if (session.user) _this.session.username = session.user.username;

    Object.keys(_this.__responseHandlerCache).forEach(function (responseMask) {

      var eventId;

      if (responseMask.indexOf(_this.session.id) >= 0) return; // dont remove new response paths

      eventId = _this.__responseHandlerCache[responseMask];

      delete _this.__responseHandlerCache[responseMask];

      _this._endpoint.data.off(eventId, function (e) {
        if (e) _this.log.error('failed detaching from old endpoint: ' + responseMask, e);
      });
    });
  };
      ...
      endpoint.data.on(responseMask, {event_type: 'set', count: 0},
        ...
        function (e, eventId) {
          ...
          if (e) {
            ...
            _this.__responseHandlerCache[responseAddress] = eventId;
  ...
  Messenger.prototype.updateRequestors = function (createComponents, destroyComponents, callback) {
    ...
    this.createRequestors(createComponents)
      ...
      .then(function () {
        ...
        var responseMask = ['_exchange','responses', _this.session.id, '*', '*', '*', '*'].join('/');
  ...
  Messenger.prototype.__prepareMessage = function (endpointName, componentName, methodName, clientId, methodDescription, args, callback) {
    ...
    _this.__validateMessage(methodDescription, args, function (e) {
      ...
      _this.__ensureResponseHandler(responseHandlerAddress, function (e) {
        ...
        if (e) return _this.__callbackImmediate(args, e); // try and run the actual response method directly
