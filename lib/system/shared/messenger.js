;(function (isBrowser) {
  ...
  function Messenger(endpoint, mesh) {
    ...
    if (endpoint.data.session.user)
      this.session.username = endpoint.data.session.user.username;
  ...
  Messenger.prototype.__createMessage = function (callbackAddress, methodDescription, args, callback) {
    ...
    if (args.length > 0 && methodDescription.synchronous && typeof args[args.length - 1] == 'function'){
