...
function MeshClient(/* opts */  /* host, port, secret, callback */) {
  ...
  // xx
  host = host || opts.hostname || opts.address;
  port = port || opts.port;
  ...
  // 'hostname' becomes 'host' for happn / TODO: call it hostname there too
  ...
  if (typeof callback == 'function') {
    log.warn('MeshClient() with callback is deprecated. Please use:');
    log.warn('...> client = new MeshClient({hostname:\'optional\', port:optional});');
    log.warn('...> client.on(\'login/accepted\', func);');
    log.warn('...> client.on(\'login/rejected\', func);');
    log.warn('...> client.login({username: \'username\', password: \'password\'});');
...
MeshClient.prototype.login = function(credentials, callback) {
  ...
  Object.keys(this.opts).forEach(function(key) {
    clone[key] = _this.opts[key];
  });

  ['username', 'password', 'secret'].forEach(function(key) {
    if (credentials[key]) {
      clone[key] = credentials[key];
    }
  });
  ...
  return initialize(this, clone, callback);
