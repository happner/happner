...
Messenger.prototype.updateRequestors = function(createComponents, destroyComponents, callback) {
  ...
  .then(function() {
    ...
    console.log('endpoint.description:::',endpoint.description);

    var responseMasks = [];

    for (var componentName in endpoint.description.components){
      var component = endpoint.description.components[componentName];
    ...
    //loop through and create endpoint listeners, ignore access denied failures
    //TODO we need the mesh to only emit a description to a client for methods
    //the client has permissions to

    console.log('response masks:::', responseMasks);

    var successfulResponseSubscriptions = 0;
    async.eachSeries(responseMasks, function(responseMask, responseMaskCB){
      ...
      endpoint.data.on(responseMask, {event_type:'set', count:0}, 
        ...
        function(e){
          if (e) {
            ...
            //we ignore unauthorized errors
            if (e.toString() != 'AccessDenied: unauthorized')
              return responseMaskCB();
            //return callback(e);
          }else{
            successfulResponseSubscriptions++;
            _this.log.$$TRACE('subscribe OK - data.on( %s', responseMask);
          }
      ...
      // if (successfulResponseSubscriptions == 0){
      //   _this.listening = false;
      //   return callback(new Error('client has no permissions to receive responses'));
      // }
  ...
  componentNames.forEach(function(componentName) {
    ...
    Object.keys(componentDescription.methods).forEach(function(methodName) {
      ...
      var requestor = function() {
        ...
        var responseAddress  = requestAddress + '/' + _this.id;

        _this.__prepareMessage(responseAddress, description, arguments[0], function(e, message){
...
Messenger.prototype.__validateMessage = function(methodDescription, args){
...
Messenger.prototype.__callbackImmediate = function(args, e, result){
  ...
  try{
    ...
    if (args.length > 1){
      ...
      var callbackFunction = args[args.length - 2];
     ...
     _this.log.error('failed running callback immediate', e);
  }
}  

Messenger.prototype.__prepareMessage = function(methodAddress, methodDescription, args, callback){

  var _this = this;

  _this.__validateMessage(methodDescription, args);

  _this.__ensureResponseHandler(methodAddress, function(e){

    if (e){
      _this.__callbackImmediate(args, e); // try and run the actual response method directly
      return callback(e);//dont do anything more
    }
      
    var message = {"callbackAddress":'/_exchange/responses' + methodAddress + '/' + _this.eventRegister++, args:[]};
...
Messenger.prototype.responseHandler = function(response, meta) {
  ...
  if (Object.keys(this.responseHandlers).length > 0){
     console.log('obj key resp handler:::', this.responseHandlers);
  }

  console.log('running resp handler:::', response, meta, this.responseHandlers);
...
Messenger.prototype.__createSubResponseHandle = function(responseAddress) {
  console.log('creating sub response handler:::', responseAddress);
...
function MeshClient(/* opts */  /* hostname, port, secret, callback */) {
  ...
  opts.hostname = opts.hostname || hostname;
...
var initialize = function(instance, opts, callback) {
  ...
  HappnClient.create({config: config, info: opts.info, Logger: opts.log, secure:opts.secure}, function(e, fbclient){
