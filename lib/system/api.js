var happn, async, Promise, Logger, utilities;
var mode = 'node';
...
var Messenger = function (endpoint, mesh){
  ...
  if (typeof mesh.log == 'object' && mesh.log.createLogger) {
    ...
    this.log = createLogger('Messenger/' + endpoint.name);
  }
...
Object.defineProperty(MeshClient, '_updateExchangeAPILayer', {
  value: Promisify(function(_this, endpointName, exchangeAPI, callback) {
    ...
    /***********
     *
     *   # INTENT (feature/mount-point)
     * 
     *   Extend to optionally support __alternative mountpoint__ for components in the 
     *   event/exchange/data/middleware api trees.
     * 
     *   That supports:
     *
     *   1. Mounting functionalities deeper into the api tree.
     *   2. Mixing methods from multiple modules into one location on the tree.
     *
     *   eg.
     *
     *      meshConfig = {
     *        components: {
     *          'irrelevant-component-name-1': {
     *            mount: '/home/nomilous/profile-info' // case 1.
     *          },
     *          'git-torrent-toolkit': {
     *            mount: '/usr/bin'                    // case 2.
     *          },
     *          'bitcoin-toolkit': {
     *            mount: '/usr/bin'                    // case 2.
     *          },
