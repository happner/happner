...
Messenger.prototype._systemEvents = function(event, data) {
  this.log.warn(event, data, data._event, data._store);
}
...
Messenger.prototype.responseHandler = function(response) {
  ...
  this.log.$$TRACE('responseHandler( ' + response._store.path);

  if (this.serializer && typeof this.serializer.__decode == 'function') {
    response.args = this.serializer.__decode(response.args, {
      req: false,
      res: true,
      addr: response._store.path,
      event: response._event,
    });
  }

  var responseHandler = this.responseHandlers[response._store.path];
  ...
  happn.create({"config":{"host":host, "port":port, "secret":secret}}, function(e, fbclient){
    ...
    client.data.get('/mesh/schema*', function(e, response){
      ...
      response.map(function(configItem){
        if (configItem._store.path == '/mesh/schema/config') {
          console.log('config', configItem);
          client._mesh.config = configItem;
        }
        else if (configItem._store.path == '/mesh/schema/description') {
          console.log('description', configItem);
  ...
  value: function(_this, description, happn, config, callback) {
    ...
    if (config.name) {
      _this._mesh.endpoints[config.name] = {
        "data": mode == 'browser' ? _this.data : _this._mesh.data,
