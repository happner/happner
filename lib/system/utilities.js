...
module.exports = function (name, config) {
  return new Util(name, config);
}

function Util(name, config) {
    ...
    try {
      ...
      if (_this.logger[guard[level]]()) {
        ...
        if (config.logTimeDelta) {
          message = ((now = Date.now()) - (_this.previous || now)) + 'ms' + delim + prefix + '(' + component + ') ' + message;
          _this.previous = now;
        }
        else {
          message = prefix + '(' + component + ') ' + message;
        }
  ...
  _this.createLogger = function(component, obj) {
    ...
    levels.forEach(function (level) {
      ...
      if (level == 'trace') {
        ...
        Object.defineProperty(obj.$$TRACE, '$happngin', {value: {ignore: true}});
      }
      else if (level == 'debug') {
        ...
        Object.defineProperty(obj.$$DEBUG, '$happngin', {value: {ignore: true}});
      }
      else {
        ...
        Object.defineProperty(obj[level], '$happngin', {value: {ignore: true}});
      }
    });
    obj.createLogger = _this.createLogger;
    return obj;
  }
