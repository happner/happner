...
module.exports.create = function(mesh, config, callback) {
  ...
  if (config.datalayer.adminPassword)
    adminUser = {
      password:config.datalayer.adminPassword
    }

  if (config.datalayer.authTokenSecret)
    config.datalayer.sessionTokenSecret = config.datalayer.authTokenSecret;
  ...
  happnServer.create({
    ...
    services: {
      ...
      data: {
        ...
        config:{
          filename: config.datalayer.filename
        }
      },
      pubsub: {
