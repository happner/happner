...
module.exports.create = function(mesh, config, callback) {
  ...
  if (config.datalayer.persist) {
    ...
    try {
      ...
      console.info('persisting %s', dbfile);
      ...
      console.warn('no read/write at %s', dbfile);
      console.warn('continuing without persistance');
  ...
  function(e, happnInstance){
    ...
    store.server.services.pubsub.on('disconnect', function(ev) {
      try {
        if (!ev.info._browser) {
          console.info('\'%s\' disconnected', (ev.info.mesh.name || 'anonymous'));
        }
      } catch (e) {}
    ...
    store.server.services.pubsub.on('authentic', function(ev) {
      try {
        if (!ev.info.browser) {
          console.info('\'%s\' connected', (ev.info.mesh.name || 'anonymous'));
        }
      } catch (e) {}
