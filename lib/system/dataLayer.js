...
module.exports.create = function(logger, config, callback) {
  ...
  // TODO: this logger will not report subsequent meshnames (logger context)
  ...
  if (config.datalayer.persist) {
    config.datalayer.dbfile = config.datalayer.dbfile || defaultDbFile;
    fs.ensureDirSync(path.dirname(config.datalayer.dbfile));
    log.info(config.datalayer.dbfile);
  }
