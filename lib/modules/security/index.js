...
// adds new permissions, overwrites any existing permissions on the same path
Security.prototype.updateGroupPermissions = function ($happn, group, permissions, callback) {

  var _this = this;

  _this.__validateRequest('addGroupPermissions', arguments, function (e) {
    if (e) return callback(e);

    _this.getGroup($happn, group.name, function (e, storedGroup) {
      if (e) return callback(e);

      var newGroup = {permissions: permissions};

      console.log(JSON.stringify(newGroup.permissions, undefined, 2));

      _.defaultsDeep(storedGroup.permissions, newGroup.permissions);

      console.log('merged=======================')
      console.log(JSON.stringify(storedGroup.permissions, undefined, 2));

      _this.__securityService.upsertGroup(storedGroup, function (e, updatedGroup) {

        if (e) return callback(e);

        console.log('updated=======================')
        console.log(JSON.stringify(updatedGroup.permissions, undefined, 2));
