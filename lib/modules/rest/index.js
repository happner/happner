...
Rest.prototype.describe = function($happn, req, res, $origin){
  ...
  if ($origin && $origin.username != '_ADMIN'){
    ...
    async.eachSeries(description.callMenu, function(accessPoint, accessPointCB){

      _this.__authorizeAccessPoint($origin, accessPoint, accessPointCB);
...
Rest.prototype.__authorizeAccessPoint = function($origin, accessPoint, callback){

  var _this = this;

  if (accessPoint.indexOf('/') == 0) accessPoint = accessPoint.substring(1, accessPoint);
  accessPoint = "/_exchange/" + accessPoint;

  _this.__securityService.authorize($origin, accessPoint, 'set', callback);
};
...
Rest.prototype.__authorize = function(req, res, $happn, $origin, uri, successful){
  ...
  if ($happn._mesh.config.datalayer.secure){ //check we need to check security
    ...
    _this.__authorizeAccessPoint($origin, uri, function(e){
