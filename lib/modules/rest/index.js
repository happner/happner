...
Rest.prototype.handleRequest = function (req, res, $happn, $origin) {
  ...
  _this.__parseBody(req, res, $happn, function(body){
    ...
    var originalUri = body.uri.toString();//to be used to check security
    ...
    if ($happn._mesh.config.datalayer.secure){ //check we need to check security
      ...
      return _this.__respond($happn, "Not implemented", null, new Error('rest not implemented for secure meshes'), res, 403);
    }

    var __callback = function(e, response){
      if (e) return _this.__respond($happn, "Not implemented", null, e, res, 500);
