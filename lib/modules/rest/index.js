...
Rest.prototype.login = function($happn, req, res){
  ...
  _this.__parseBody(req, res, $happn, function(body){
      ...
      var client = new Mesh.MeshClient($happn._mesh.config.datalayer);

      client
      .login({secure: true, username:body.username, password:body.password})
      .then(function(){
        _this.__respond($happn, "Logged in ok", {token:client.token}, null, res);
      })
      .catch(function(e){
        _this.__respond($happn, "Failure logging in", null, e, res);
        }
      );
    }
  );
